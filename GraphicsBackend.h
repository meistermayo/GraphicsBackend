...
protected:
	...
	const GraphicsDevice& GraphicsBackend_Base::privGetDevice() const { return mDev; }
	const GraphicsContext& GraphicsBackend_Base::privGetContext() const { return mCon; }
...
#endif
#ifdef BACKEND_OGL
	void LoadTexture(const std::string& filepath, bool ComputeMip = false, size_t miplevel = 0, uint32_t filterflags = 0);
	void LoadColorTexture(Vect color);
	void CreateSampleState(uint32_t filter = 0, uint32_t isotropic_level = 0);
#endif
...
protected:
	...
	virtual void privSetClearColor(float r, float g, float b, float a = 1.0f) override;
	...
	virtual const bool privGetKeyState(KEY_CODE key) const override;

	virtual const int privGetWindowWidth() const override;
	virtual const int privGetWindowHeight() const override;

public:
	virtual void OnKey(KEY_CODE key, bool down) override;
	virtual void OnMouseMove(int x, int y) override;
	virtual void OnMouseWheel(int w) override;
};
#endif
